...
class local_trainer(object):
    ...
    def update_weights_scaffold_unsupervised(self, model, history_model, c_global, c_local, labeled_data_dict, unlabeled_data_dict, u, logit_threshold, current_epoch, pseudo_dict, T):
        ...
        kld_criterion = nn.KLDivLoss().to(self.device)
        ...
        for iter in range(int(self.args.local_epochs)):
            ...
            if iter == 0 and history_model and len(unlabeled_data_dict['data']) != 0:
                ...
                pseudo_hist_label_arr = np.zeros([10, len(unlabeled_data_dict['data']), 4])
                pseudo_nl_label_arr = np.zeros([10, len(unlabeled_data_dict['data']), 4])
                pseudo_nl_hist_label_arr = np.zeros([10, len(unlabeled_data_dict['data']), 4])
                for aug_idx in range(10):
                    ...
                    logit_list, logit_nl_list = [], []
                    history_logit_list, history_logit_nl_list = [], []
                    pseudo_real_label = []
                ...
                argmax_pseudo_logits, argmax_hist_pseudo_logits = np.argmax(avg_pseudo_logits, axis=1), np.argmax(avg_hist_pseudo_logits, axis=1)
                ...
                max_std = []
                for idx, item in enumerate(max_idx):
                    max_std.append(out_std[idx][item])
                max_std = np.array(max_std)
                
                out_hist_std, out_hist_prob = np.std(pseudo_hist_label_arr, axis=0), np.mean(pseudo_hist_label_arr, axis=0)
                max_hist_value, max_hist_idx = np.max(out_hist_prob, axis=1), np.argmax(out_hist_prob, axis=1)
                max_hist_std = []
                for idx, item in enumerate(max_hist_idx): max_hist_std.append(out_hist_std[idx][item])
                max_hist_std = np.array(max_hist_std)
                
                selected_idx = (max_value >= tau_p) * (max_std < kappa_p)
                selected_hist_idx = (max_hist_value >= tau_p) * (max_hist_std < kappa_p)
                # selected_idx = selected_idx * selected_hist_idx
                
                pseudo_target, pseudo_hist_target = max_idx[selected_idx], max_hist_idx[selected_hist_idx]
                pseudo_maxstd, pseudo_hist_maxstd = max_std[selected_idx], max_hist_std[selected_hist_idx]
                pseudo_idx = np.where(selected_idx == True)[0]
                pseudo_hist_idx = np.where(selected_hist_idx == True)[0]
                # ----------------------------------------------
                ...
                if len(pseudo_target) > 0:
                    ...
                    for class_idx in range(4):
                        ...
                        if len(np.where(pseudo_target==class_idx)[0]) > 0:
                            ...
                            sorted_maxstd_idx = sorted_maxstd_idx[:3]
                            
                            for idx in sorted_maxstd_idx:
                                labeled_data_dict['data'] = np.append(labeled_data_dict['data'], np.expand_dims(unlabeled_data_dict['data'][pseudo_idx[idx]], axis=0), axis=0)
                                labeled_data_dict['label'] = np.append(labeled_data_dict['label'], class_idx)
                                final_pseudo_list.append(pseudo_idx[sorted_maxstd_idx[0]])
            ...
            for batch_idx, batch_data in enumerate(train_dataloaders):
                ...
                if batch_idx < len(unlabeled_dataloader):
                    ...
                    # logits1 = F.log_softmax(logits1)
                    logits1 = F.softmax(logits1)
                    logits2 = F.softmax(logits2)
                    loss += u*mse_criterion(logits1, logits2)
    ...
    def update_weights_scaffold_crest(self, model, history_model, c_global, c_local, labeled_data_dict, unlabeled_data_dict, u, logit_threshold, current_epoch, pseudo_dict, T):
        lr = float(self.args.learning_rate) / np.power(2, (int(current_epoch/100)))
        optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-04, betas=(0.9, 0.98), eps=1e-9)
        global_model = copy.deepcopy(model)
        mse_criterion = nn.MSELoss().to(self.device)
        ...
        for iter in range(int(self.args.local_epochs)):
            ...
            if iter == 0 and history_model and len(unlabeled_data_dict['data']) != 0:
                ...
                for idx, freq_idx in enumerate(freq_arg):
                    ...
                    sample_dict[key] = int(np.power(ratio, 1/2) * 5)
            ...
            for batch_idx, batch_data in enumerate(train_dataloaders):
                ...
                if len(pseudo_data_dict['label']) > 0:
                    if batch_idx < len(unlabeled_dataloader):
                        ...
                        loss += u*criterion(logits1, unlabel_y_list[batch_idx].to(self.device))*len(logits1)/16
